...
1. **Tre griglie di ricerca-sequenza** (cifre, lettere latine, cirilliche): 25 righe × 12 colonne, contenute rigorosamente entro una singola pagina A4 (bug corretto: con l'altezza di riga usata in una versione precedente l'ultima riga finiva da sola su una seconda pagina — verificato renderizzando il documento vero, non solo leggendo il codice). La sequenza bersaglio compare **esattamente 16 volte** in totale — 8 in orizzontale + 8 in verticale, mai un numero casuale — verificato ad ogni generazione, rigenerando la griglia se il conteggio non torna esatto. Il conteggio esatto non viene però scritto nelle istruzioni al paziente, che vede solo "(orizzontale e verticale)".
...
**Fonemico combinato** (schema "lettera iniziale + numero di lettere", uno dei 7 schemi che ciclano): stessa validazione applicata — 71 combinazioni su 72 (18 lettere × lunghezza 4-7) hanno almeno 15 parole italiane reali; l'unica scartata ("iniziale I, 4 lettere") ne aveva solo 8. Gli altri due schemi combinati ("lettera+categoria", "categoria+lunghezza") usano una categoria semantica invece di un vincolo puramente ortografico, quindi non sono verificabili nello stesso modo con un dizionario generico: restano su lunghezze moderate (4-8) scelte per prudenza, non verificate parola per parola.
...
**Perché non unDraw**: è stata valutata (su richiesta) e scartata con dati alla mano, non per preferenza stilistica. unDraw è una libreria di ~1362 illustrazioni pensate per scene astratte da sito/app (es. "Quitting time", "User flow", "Robotics", "Scrum board"), non per oggetti concreti. Verificata con lo stesso metodo usato per OpenMoji contro gli stessi 1371 termini: solo l'86 (6,3%) ha un titolo/tag che contiene tutte le parole del termine cercato, e anche tra questi molti abbinamenti sono semanticamente sbagliati (es. "bear"→"Meeting", "cat"→"Product hunt"). Una copertura reale utilizzabile è quindi stimabile ben sotto il 5%, contro il 41% verificato di OpenMoji.
...
- Serie attentive alternate: le prime 4 stimolazioni sono fisse (A-nero, B-rosso, 1-rosso, 2-nero) per scelta esplicita, anche se questo riduce leggermente la varietà "di riscaldamento" a inizio sequenza; dalla 5ª stimolazione in poi tutte le combinazioni simbolo/colore restano comunque raggiungibili, quindi le regole dei livelli più alti (che richiedono lettere e numeri in entrambi i colori) non sono compromesse.
